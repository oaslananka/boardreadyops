...
export async function generateMetadata({ params }: PageProps) {
  ...
  return { title: await resolveRunPageTitle(runId, "Artifacts", viewer.authorizeRepository) };
}
...
export default async function ArtifactsPage({ params, searchParams }: PageProps) {
  ...
  return (
    <RunPageFrame run={result.run} active="artifacts" liveRefresh={shouldLiveRefreshRun(result.run.investigationState)}>
