...
While a run is current, reconciling, or stale, every investigation route uses a reconnectable five-second server refresh. Refreshes are paused while the page is hidden or offline, resume immediately when visibility or network connectivity returns, and stop once the run reaches a terminal or operator-only state. The browser receives only the same normalized, tenant-scoped dashboard model used by a normal page request; workflow logs, source, webhook payloads, credentials, and unredacted runner output are not streamed to the page.
