...
The current public npm package is `boardreadyops@1.59.0`. It declares Node.js 22.14+
...
```bash
npx -y boardreadyops@1.59.0 --help
```

## Binary Installers

The shell and PowerShell installers require a GitHub Release that contains the
platform binary asset and `SHA256SUMS`. The `v1.59.0` release contains the Linux,
...
example `BOARDREADYOPS_VERSION=1.59.0`. The Homebrew formula is populated with the `v1.59.0` macOS and Linux
...
and maintainer process are chosen. It points at `v1.59.0` release binaries and
