...
- **Status:** `Partial`
- **Remaining:** `/api/v1/billing/checkout` and `/portal` now create real Stripe Checkout/Billing Portal sessions (`apps/web/lib/stripe-billing-client.ts`, official `stripe` SDK) gated behind a new `BILLING_MODE` environment switch (`apps/web/lib/billing-mode.ts`) that defaults to `marketplace_free` — today's HTTP 410 behavior — so nothing changes in production until an operator deliberately sets `BILLING_MODE=stripe` or `both` and configures `STRIPE_SECRET_KEY` alongside the pre-existing `STRIPE_*_PRICE_ID` table. Once flipped, `checkout.session.completed` can fire for the first time, activating the previously-dormant `linkStripeCustomer`/`applyStripeSubscriptionEvent` projection. Tenant identity for the Stripe path reuses the pre-existing convention (`viewer.session.login`, matching `BillingStore.forecastContributors`'s usage on the read-only Marketplace billing page) rather than inventing a new one. Price→tier mapping reuses the pre-existing `STRIPE_*_PRICE_ID` env-driven table — those price ids are illustrative and must be confirmed against the real Stripe dashboard before production use. Newly closed: the pre-existing `handoffLinksEnabled(tier)` entitlement was defined but never checked anywhere — `POST /api/v1/external-review-links` let any authenticated caller create unlimited external (CM/fab) review links on every plan tier, including free. Now gated: the route resolves the repository's installation `plan_tier` and returns 403 with an upgrade message on tiers without handoff links, verified against a real disposable Postgres instance, not just mocked. Still open: (1) Coverage is unit-level only for the Stripe checkout/portal routes (mocked DB/executor, injected Stripe client); no real Stripe test-mode account has exercised them end-to-end. (2) A tier change does not itself call `entitlement-store.ts`'s `applyWatchAllowance` — `billing_customers.tenant_id` has no established mapping back to an `installations.id` (it is a bare, unconstrained text column; the checkout/portal routes just added populate it with the GitHub login), so wiring this is a real, undesigned access-control decision, not a one-line call, mirroring the pre-existing Marketplace path's same gap. (3) The other entitlements the product plan proposes (cross-run finding history, shared org policy, waiver approval workflow) have no existing feature surface to gate yet.
- **Scope:** Tiered plan entitlements (Free / Team / Business / Enterprise), Stripe signature verification, self-serve checkout/portal, and marketplace billing models.
- **Code & Test Evidence:** `ADR-0014`, `packages/cloud-core/src/stripe-service.ts`, `packages/db/src/billing-store.ts`, `packages/db/migrations/0060_stripe_subscription_event_ordering.sql`, `apps/web/app/api/v1/billing/webhook/route.ts`, `apps/web/app/api/v1/billing/checkout/route.ts`, `apps/web/app/api/v1/billing/portal/route.ts`, `apps/web/lib/billing-mode.ts`, `apps/web/lib/stripe-billing-client.ts`, `apps/web/app/api/v1/external-review-links/route.ts`.
...
- **Code & Test Evidence:** `apps/web/app/runs/`, `apps/web/app/reviews/`, `apps/web/app/settings/`, `apps/web/lib/run-dashboard.ts`, `apps/web/components/run-investigation.tsx`, `tests/e2e/qa-audit.spec.ts`, `src/core/rule-registry.ts`, `src/report/review-comment.ts`, `packages/db/migrations/0062_finding_category.sql`.
