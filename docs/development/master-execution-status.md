...
- **Code & Test Evidence:** `apps/web/lib/auth.ts`, `apps/web/lib/api-token-store.ts`, `packages/cloud-core/src/entitlements.ts`.
...
- **Code & Test Evidence:** `ADR-0014`, `packages/cloud-core/src/stripe-service.ts`, `packages/db/src/marketplace-billing-store.ts`.
