...
**Architecture:** A testable Node route-discovery module fetches signed-in server-rendered pages with `brops_session`, allowlists representative product routes, and writes a secret-free manifest. A pinned `@unlighthouse/core` runner consumes that manifest and cookie at runtime, drives the installed Chrome through the programmatic Unlighthouse API, and generates a static report; package scripts and a manual read-only GitHub Actions workflow run the audit and upload a private artifact.
...
- [x] **Step 3: Implement the pure route policy**
...
- [x] **Step 4: Add authenticated fetch/fail-closed tests**
...
- [x] **Step 5: Implement authenticated discovery**
...
- [x] **Step 6: Re-run the focused test and confirm GREEN**

Run: `corepack pnpm exec vitest run tests/unit/scripts/unlighthouse-auth-routes.test.ts`
Expected: PASS.

- [x] **Step 7: Commit the route-discovery slice**
...
**Interfaces:**
- `scripts/unlighthouse-authenticated.mjs` reads `BROPS_SESSION` and optional `BROPS_UNLIGHTHOUSE_SITE`, writes `.unlighthouse/authenticated-routes.json`, then runs `@unlighthouse/core` through `createUnlighthouse` and `generateClient`.
...
- [x] **Step 2: Run the focused orchestration test and confirm RED**
...
- [x] **Step 3: Add the pinned dependency and ignored output**

Use pinned `@unlighthouse/core@0.18.0`. The full `unlighthouse@0.18.0` package was rejected by the repository provenance no-downgrade policy and is intentionally not allowlisted.
...
- [x] **Step 5: Implement the orchestration script**

Validate HTTPS for the default production site, permit HTTP only for loopback test sites, call `discoverAuthenticatedRoutes`, atomically write the route manifest, and execute the pinned programmatic Unlighthouse core. A `--routes-only` mode stops after discovery; `--debug` enables Unlighthouse debug/headful behavior without persisting browser state.

- [x] **Step 6: Re-run focused tests and static checks**
...
- [x] **Step 7: Commit the executable audit slice**
...
- Produces a private GitHub Actions artifact named `authenticated-unlighthouse-${{ github.run_id }}` containing only `.unlighthouse/authenticated/` and the secret-free route manifest.
...
Use `ubuntu-24.04`, Node `24.20.0`, a 30-minute timeout, concurrency `authenticated-ui-audit`, dependency install through `deps:install-ci`, and `corepack pnpm run qa:unlighthouse:auth`. Fail explicitly when the session secret is blank or expired. Upload the report with the repository's pinned `actions/upload-artifact` SHA and `retention-days: 7`.

- [x] **Step 4: Verify workflow contracts and repository workflow lint**
...
- [x] **Step 2: Run a synthetic authenticated end-to-end fixture**
...
- [x] **Step 4: Verify Unlighthouse CLI/config wiring without a production secret**

Verify the pinned core dependency through the focused runner tests and the real loopback Chrome canary.
...
- [x] **Step 5: Commit documentation and plan/spec**
