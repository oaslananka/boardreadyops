...
The production BoardReadyOps GitHub App receives no new GitHub App permission for the canary. It keeps the ordinary target-repository profile declared in `githubAppPermissionProfile` (`packages/cloud-core/src/github-capabilities.ts`), which is the single source of truth this runbook, the `/setup` page, and the setup API all read: Metadata read, Checks read/write, Actions read/write, Pull requests read/write, Contents read/write, Workflows read/write, and Issues read/write. No organization or account permissions are requested, and repository writes are confined by `GitHubMutationService` to `boardreadyops.yml`, `.github/workflows/readiness-runner.yml`, and `.boardreadyops/**` on a new branch opened as a pull request.

> **Changed 2026-09-14.** This section previously instructed operators to refuse any installation requesting Contents, which contradicted both the capability model in `github-capabilities.ts` and the mutation service that has always required `contents: write` to open a setup, waiver, or remediation pull request. See [ADR-0018](../architecture/adr/0018-github-app-permission-profile.md).

Before installation, verify the live App registration rather than relying only on repository documentation. The requested permissions and subscribed events must match the declared profile. Do not install the App when the live registration requests repository administration, organization, account, secret, or any write access the profile does not declare — a grant the product cannot explain is a grant to refuse. A Contents or Workflows write grant is expected; `missingDeclaredPermissions()` reports any declared grant an installation is missing, and the `/setup` page renders that reading for the installation owner.
