...
| **Workflow Dispatch** | Trigger `.github/workflows/readiness-runner.yml` on PR events and authorized re-run/release-preview actions | `actions: write` | `workflow_run` (`completed`, `in_progress`) | Triggers only the exact repository workflow with exact PR head/base SHA and run-attempt binding. Release preview never publishes a tag or GitHub Release. | Dispatches logged with trigger reason and commit SHA. |
...
## 6. Production Change Procedure & GitHub Permission Update Semantics

When updating GitHub App permissions in production:
1. Apply GitHub's update semantics:
   - **Permission and webhook removals take effect immediately** across all installations without requiring user intervention.
   - **New or broader permissions require installation-owner approval** before taking effect on existing installations.
2. Maintain separate development and production Apps so experimental features never trigger broad permission approval prompts prematurely.
3. Configure and test webhook verification and capability fallback paths whenever permission scopes change.
