-- Persist the immutable trust-mode decision used when each release run is enqueued.
-- The snapshot is derived inside the guarded transactional producer so retries cannot
-- rewrite historical execution context and the dashboard can explain reduced-trust runs.

alter table release_runs
  add column if not exists trust_mode text not null default 'standard',
  add column if not exists safe_mode_reasons text[] not null default '{}'::text[];

alter table release_runs
  drop constraint if exists release_runs_trust_mode_valid,
  add constraint release_runs_trust_mode_valid
    check (trust_mode in ('standard', 'safe')),
  drop constraint if exists release_runs_safe_mode_reasons_valid,
  add constraint release_runs_safe_mode_reasons_valid
    check (
      ...
      and safe_mode_reasons = array_remove(array[
        case when 'draft-pull-request' = any(safe_mode_reasons) then 'draft-pull-request' end,
        case when 'fork-pull-request' = any(safe_mode_reasons) then 'fork-pull-request' end,
        case when 'private-repository' = any(safe_mode_reasons) then 'private-repository' end
      ]::text[], null)
    ),
  drop constraint if exists release_runs_safe_mode_consistent,
  add constraint release_runs_safe_mode_consistent
    check (
      (trust_mode = 'standard' and cardinality(safe_mode_reasons) = 0)
      or (trust_mode = 'safe' and cardinality(safe_mode_reasons) between 1 and 3)
    );

create index if not exists release_runs_trust_mode_started_idx
  on release_runs(trust_mode, started_at desc, id desc);
