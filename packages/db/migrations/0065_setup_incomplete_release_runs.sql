-- Mark unconfigured repositories before the Check Run dispatch transition.
-- The repository setup revision and its readiness statuses are read in the same enqueue
-- transaction, so the outbox payload and release-run snapshot cannot disagree about whether
-- setup is actually runnable. An open-but-unvalidated setup PR remains incomplete.
...
declare
  ...
  v_setup_workflow_status text;
  v_setup_config_status text;
  v_candidate record;
...
begin
  select installations.id,
         repositories.id,
         repositories.current_setup_revision_id,
         setup_revision.workflow_status,
         setup_revision.config_status
    into v_installation_id,
         v_repository_id,
         v_setup_revision_id,
         v_setup_workflow_status,
         v_setup_config_status
    from repositories
    join installations on installations.id = repositories.installation_id
    left join repository_setup_revisions setup_revision
      on setup_revision.id = repositories.current_setup_revision_id
  ...
  if v_setup_revision_id is null
     or v_setup_workflow_status is distinct from 'ready'
     or v_setup_config_status is distinct from 'ready' then
    v_outbox_payload := jsonb_set(
      v_outbox_payload,
      '{action,setupIncomplete}',
      'true'::jsonb,
      true
    );
  end if;
