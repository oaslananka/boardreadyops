...
export function sanitizeAndExtractArchive(buffer: Buffer, limits: ArchiveLimits = {}): SanitizeArchiveResult {
  ...
  for (let entryIdx = 0; entryIdx < totalEntries; entryIdx++) {
    ...
    if (
      ...
      normalizedName.split("/").some((seg) => seg === "..")
    ) {
    ...
    if (totalExtractedBytes + uncompressedSize > maxTotalExtracted) {
      throw new ArchiveSanitizerError(
        "DECOMPRESSION_BOMB",
    ...
    // Extract data from local header
    ...
    if (method === 0) {
      extractedData = Buffer.from(compressedSlice);
    } else if (method === 8) {
      try {
        extractedData = inflateRawSync(compressedSlice, {
          maxOutputLength: uncompressedSize + 1024,
        });
      } catch (err) {
        throw new ArchiveSanitizerError(
          "CORRUPTED_ARCHIVE",
    ...
    totalExtractedBytes += extractedData.length;
    if (totalExtractedBytes > maxTotalExtracted) {
      throw new ArchiveSanitizerError(
        "DECOMPRESSION_BOMB",
        `Total extracted bytes exceeded limit of ${maxTotalExtracted}`,
      );
    }
