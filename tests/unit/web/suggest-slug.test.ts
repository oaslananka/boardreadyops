...
describe("suggestSlug", () => {
  ...
  it("produces something the input's own pattern accepts", () => {
    const pattern = /^[a-z0-9-]+$/u;
    for (const name of ["Acme Hardware", "Motor Control v2.1", "Ünïcödé Board", "  spaced  out  "]) {
      const slug = suggestSlug(name);
      if (slug !== "") expect(slug).toMatch(pattern);
    }
  });
